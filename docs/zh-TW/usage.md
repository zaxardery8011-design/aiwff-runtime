...
確定 daemon 有在跑（終端機 `npm run web`，看到 `AIWFF Runtime listening on http://127.0.0.1:3100`），瀏覽器打開那個網址。控制台頂端目前有 8 個分頁：**HUD 儀表**、**對話 / 新任務**、**任務**、**進度 / 事件**、**收件匣 / 卡住**、**記憶**、**紀錄**、**診斷 / 設定**。

### 2.1 建立任務（「對話 / 新任務」分頁）

切到 **對話 / 新任務** 分頁，下方輸入列打一句你要它做的事、按 Enter（或點送出按鈕）就送出。送出後它會即時回你「任務已建立」，接著顯示待執行、執行中、完成與 artifact 路徑等狀態更新：

![WebUI 對話 / 新任務分頁 — 建立任務與狀態更新](../images/webui-03-chat-new-task.png)

> 背後發生的事：送出等於對 `POST /api/tasks` 建了一筆任務，daemon 立刻派 mock worker 去跑，產出寫進 `data/artifacts/`（詳見 §4 的檔案地圖）。

### 2.2 看全局狀態（「HUD 儀表」分頁）

**HUD 儀表** 是一眼看全局的首頁：中央顯示系統狀態，左側是執行狀態與工作佇列，右側是事件串流與待注意項目。你可以在這裡快速確認 daemon 是否在線、現在有幾個任務、收件匣與記憶檔狀態是否正常。
...
- **任務**：列出任務清單、狀態、建立時間與 artifact 路徑，適合找某一筆任務。
- **進度 / 事件**：左側是事件流，右側是每個任務的進度紀錄，適合追查任務跑到哪一步。

![WebUI 進度 / 事件分頁 — 事件流與任務進度](../images/webui-02-task-progress.png)
...
用真的 Telegram bot token 收發時，流程是：你傳文字給 bot，daemon 建立任務並先回覆收到；任務完成後，bot 再推送完成或失敗通知。bot token 與 chat id 需要你自己在 Telegram 取得，本專案只讀取 `.env` 裡的 `TG_BOT_TOKEN` 與 `ADMIN_TG_CHAT_ID`，不會代替你建立 bot。

若要確認端到端是否接通，請先用 `/start` 和 `/tasks` 驗證 bot 回應，再送一個短任務觀察 WebUI 是否同步出現該任務。若 WebUI 有任務但 Telegram 沒通知，優先檢查 `.env` 的 chat id 是否就是你目前對 bot 發訊息的那個帳號。
