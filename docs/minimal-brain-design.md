...
L6  多節點編排  ← 完整版 AIWFF 在這層（主控節點 + 第二節點 + 外部 reviewer + CLI worker）
```
...
[Claude] 讀 CLAUDE.md，知道自己是誰、能用什麼工具
         ...
         ▼
[Claude] 自主執行：
  1. Glob("./workspace/**/*.pdf")  → 找到 12 個 PDF
  2. 整理成 Markdown 表格
...
`--dangerously-skip-permissions` 模式需要 Claude 訂閱帳號，免費帳號無法使用。
...
|---|---|---|
| 大腦配置 | 多檔治理設定（身份、邊界、操作規則） | CLAUDE.md（身份 + 邊界 + 規則，你直接改） |
...
| 多節點 fleet | 主控節點 + 第二節點 + 跨機派工 | **✗ 唯一拿掉的功能**（單機） |
| 外部諮詢 | 外部 LLM reviewer + CLI reviewer | 選配未實作：外部 LLM reviewer / CLI reviewer |
| 自我審查 | multi-juror verify（3+ agent 投票） | 現況只檢查 artifact 存在，Claude 自我回審是未來強化 |
